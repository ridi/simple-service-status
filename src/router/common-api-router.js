...
module.exports = [
  {
    ...
    handler: (request, reply) => {
      ...
      if (process.env.ALLOWED_IP && !process.env.ALLOWED_IP.includes(clientIP)) {
        ...
        return reply(new SSSError(SSSError.Types.FORBIDDEN_IP_ADDRESS, { remoteAddress: clientIP }));
      }
      if (!request.payload.username || !request.payload.password) {
        return reply(new SSSError(SSSError.Types.AUTH_MISSING_PARAMS));
      }
      return User.find({ username: request.payload.username }).then((account) => {
        if (!account || account.length === 0 || !authUtil.comparePassword(request.payload, account[0].password)) {
          return reply(new SSSError(SSSError.Types.AUTH_INVALID_PARAMS));
        }
